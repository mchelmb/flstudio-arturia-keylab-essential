...
class ArturiaInputControls:
    ...
    def ProcessKnobInput(self, event, knob_index, delta):
        # Context-sensitive Channel Rack strip controls (vol / pan / target mix)
        # when the Channel Rack window is focused. Lives alongside the menu path
        # (Channel > Volume / Panning / Target Mix) so both deliberate and live
        # control are available.
        if (getattr(config, 'ENABLE_CHANNEL_RACK_CONTEXT_KNOBS', True)
                and knob_index in (0, 1, 2)
                and self._channel_rack_is_focused()):
            self._process_channel_rack_context_knob(knob_index, delta)
            return self
    ...
    def _channel_rack_is_focused(self):
        try:
            return bool(ui.getFocused(midi.widChannelRack))
        except Exception:
            return False

    def _process_channel_rack_context_knob(self, knob_index, delta):
        """Knob 0 = channel volume, 1 = pan, 2 = target mixer track."""
        try:
            ch = channels.selectedChannel()
            if ch < 0:
                return
            if knob_index == 0:
                vol = max(0.0, min(1.0, channels.getChannelVolume(ch) + (delta / 100.0)))
                channels.setChannelVolume(ch, vol)
                self._display_hint(channels.getChannelName(ch)[:16],
                                   'Vol %d%%' % int(vol * 100))
            elif knob_index == 1:
                pan = max(-1.0, min(1.0, channels.getChannelPan(ch) + (delta / 100.0)))
                channels.setChannelPan(ch, pan)
                self._display_hint(channels.getChannelName(ch)[:16],
                                   'Pan %+d%%' % int(pan * 100))
            elif knob_index == 2:
                max_track = mixer.trackCount() - 2
                prev = channels.getTargetFxTrack(ch)
                target = prev + delta
                if target < 0:
                    target = max_track
                elif target > max_track:
                    target = 0
                channels.setTargetFxTrack(ch, target)
                label = 'MASTER' if target == 0 else ('Mix %d' % target)
                self._display_hint(channels.getChannelName(ch)[:16], label)
        except Exception as exc:
            log('ChannelRackCtx', 'knob %d failed: %s' % (knob_index, repr(exc)))
