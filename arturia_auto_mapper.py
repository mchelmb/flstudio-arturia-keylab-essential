...
def export_scan(plugin_name, params):
    ...
    try:
        ...
        if not latency_gate.allow_disk_io():
            return
    except ImportError:
        pass
    try:
...
class AutoMapper:
    ...
    def Idle(self, now_ms):
        ...
        Latency gate: while recording (or strict live mode) new discovery is suppressed so the
        MIDI thread stays responsive for notes and transport. An in-progress chunked scan is
        still allowed to finish (it is already bounded) but no new settle commits are started.
        """
        try:
            from arturia_menu import latency_gate
            if not latency_gate.allow_discovery():
                # Drop any pending settle so we don't resume a heavy scan the moment recording ends
                # unless the plugin focus actually changes again.
                if self._scan is None:
                    self._pending_key = None
                    self._pending_since_ms = None
                    return
                # Allow an already-running chunked scan to drain, but do not start a new one.
        except ImportError:
            pass
