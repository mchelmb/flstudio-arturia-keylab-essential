...
try:
    ...
    VK_MAP = {
        ...
        'tab': 0x09, 'enter': 0x0D, 'return': 0x0D, 'escape': 0x1B, 'space': 0x20,
    ...
    # Keys that live on the "extended" block of a standard keyboard (arrow cluster, Ins/Del/Home/
...
class Actions:
    ...
    def fl_windows_shortcut(key, shift=0, ctrl=0, alt=0):
        ...
        if PYKEYS_ENABLED:
            ...
            return True
        # Fallback: ctypes SendInput (works in sub-interpreters)
        if CTYPES_SENDINPUT_AVAILABLE:
            ...
            # Modifiers down
            if shift:
                inputs.append(INPUT(1, KEYBDINPUT(VK_MAP['shift'], 0, 0, 0, 0)))
            if ctrl:
                inputs.append(INPUT(1, KEYBDINPUT(VK_MAP['ctrl'], 0, 0, 0, 0)))
            if alt:
                inputs.append(INPUT(1, KEYBDINPUT(VK_MAP['alt'], 0, 0, 0, 0)))
            # Key down/up
            key_flag = KEYEVENTF_EXTENDEDKEY if key.lower() in EXTENDED_KEYS else 0
            inputs.append(INPUT(1, KEYBDINPUT(vk, 0, key_flag, 0, 0)))
            inputs.append(INPUT(1, KEYBDINPUT(vk, 0, key_flag | KEYEVENTF_KEYUP, 0, 0)))
            # Modifiers up
            if alt:
                inputs.append(INPUT(1, KEYBDINPUT(VK_MAP['alt'], 0, KEYEVENTF_KEYUP, 0, 0)))
            if ctrl:
                inputs.append(INPUT(1, KEYBDINPUT(VK_MAP['ctrl'], 0, KEYEVENTF_KEYUP, 0, 0)))
            if shift:
                inputs.append(INPUT(1, KEYBDINPUT(VK_MAP['shift'], 0, KEYEVENTF_KEYUP, 0, 0)))
