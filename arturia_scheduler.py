...
<<<<<<< HEAD
            try:
                ...
                # A single stale/misbehaving task must never crash the whole idle loop. This
                # matters most after a long gap (e.g. alt-tabbing away and back for a while) -
                # every task that became due during the gap fires in one burst right here, and one
                # bad task taking the whole loop down with it (and this runs before any of our own
